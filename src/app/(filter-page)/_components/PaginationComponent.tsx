...
function PaginationComponent({ paginationData }: PaginationComponentProps) {
  ...
  const renderPageNumbers = () => {
    ...
    pages.push(
      <div
        key={1}
        className={`rounded-full flex min-w-7 h-7 items-center justify-center border border-solid ${
  ...
  return (
    <div className="flex gap-2 items-center text-[12px]">
      ...
      <Image
        ...
        className={`cursor-pointer ${!paginationData.has_previous_page ? 'opacity-50' : ''}`}
      ...
      <Image
        ...
        width={15}
        height={15}
        className={`cursor-pointer ${!paginationData.has_next_page ? 'opacity-50' : ''}`}
