  ...
  return (
    ...
    <div className=''>
        <div className='lg:mb-[8px] '>
            <h2 className={cn('rubik_semibold text-[22px] max-sm:text-[18px] text-left leading-[26px] text-gunmetal',actualPriceClass?? null)}> <span className={`${isHome ? 'text-[12px]': 'text-[14px]'}`}>AED</span> {data?.current_price }</h2>
        </div>
        { data?.is_offer ? <div className='flex flex-row items-center gap-[8px] '>
            ...
            <div className='flex  justify-center items-center'>
                <h6 className='line-through text-center  rubik_semibold leading-[16px] text-gunmetal text-[12px]'>  <span className={`${isHome ? 'text-[14px]': ''}`}>AED</span>  {data?.actual_price}</h6>
            </div>
